...
class SalesOrderPage extends StatefulWidget {
  const SalesOrderPage({super.key, this.appBarLeading});
...
class _SalesOrderPageState extends State<SalesOrderPage>
  ...
  static const String _activeDraftKey = 'sales_order_active_draft';
  ...
  String get _documentTitle => 'Sales Order';
  String get _documentTitleLower => 'sales order';
  String get _postDialogTitle => 'Post Sales Order';
  String get _postButtonLabel => 'Post Sales Order';
  String get _itemsTitle => 'Sales Order Items';
  String get _summaryTitle => 'Sales Order Summary';
  Color get _accentColor => const Color(0xFF2362EC);
  ...
  Future<void> _saveTemporaryData() async {
    ...
    try {
      ...
      final key = 'sales_order_temp_$customerId';
  ...
  Future<void> _recallTemporaryData() async {
    ...
    try {
      ...
      if (savedData == null) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('No saved data found for this customer'),
            backgroundColor: Colors.orange,
          ),
        );
        return;
                            ...
                            if (selectedRemainingPaymentMethod != null) ...[
                              ...
                              FilledButton.icon(
                                ...
                                style: FilledButton.styleFrom(
                                  ...
                                  foregroundColor: const Color(0xFF2362EC),
                                ),
                              ),
        ...
        result =
            await showDialog<Map<String, dynamic>>(
              ...
              builder: (dialogContext) => AlertDialog(
                title: Row(
                  children: [
                    Icon(Icons.payment_rounded, color: const Color(0xFF2362EC)),
                    const SizedBox(width: 12),
  ...
  Future<void> _openPostSalesOrderDialog() async {
    ...
    final paymentModes = await ApiService.getPaymentModes();
    String? selectedPrimaryPaymentCode;
    final primaryAmountController = TextEditingController(
      text: total.toStringAsFixed(2),
    );
    String? selectedRemainingPaymentCode;
    List<Map<String, dynamic>>? selectedPaymentMethodsForPDF;
      ...
      builder: (context) {
        return StatefulBuilder(
          builder: (context, setState) {
            final theme = Theme.of(context);
            final enteredPrimaryAmount =
                double.tryParse(primaryAmountController.text) ?? 0.0;
            final remainingBalance = (total - enteredPrimaryAmount).clamp(
              0.0,
              double.infinity,
            );
            final hasRemainingBalance =
                selectedPrimaryPaymentCode != null && remainingBalance > 0.01;
                            ...
                            Container(
                              ...
                              child: Column(
                                ...
                                children: [
                                  ...
                                  const SizedBox(height: 16),
                                  // Payment Mode Selection Dropdown (Optional)
                                  DropdownButtonFormField<String?>(
                                    isExpanded: true,
                                    value: selectedPrimaryPaymentCode,
                                    decoration: InputDecoration(
                                    ...
                                    onChanged: (value) {
                                      setState(() {
                                        _paymentError = null;
                                        selectedPrimaryPaymentCode = value;
                                        if (value != null) {
                                          primaryAmountController.text =
                                              total.toStringAsFixed(2);
                                        }
                                        selectedRemainingPaymentCode = null;
                                      });
                                    },
                                  ),
                                  if (selectedPrimaryPaymentCode != null) ...[
                                    const SizedBox(height: 12),
                                    TextField(
                                      ...
                                      decoration: InputDecoration(
                                        labelText:
                                            'Amount for ${paymentModes.firstWhere((pm) => pm['PaymentCode'] == selectedPrimaryPaymentCode, orElse: () => {})['PaymentMode'] ?? 'Selected Payment'}',
                                        prefixText: 'Rs. ',
                                        border: OutlineInputBorder(
                                          borderRadius:
                                    ...
                                    if (hasRemainingBalance) ...[
                                      ...
                                      DropdownButtonFormField<String?>(
                                        isExpanded: true,
                                        value: selectedRemainingPaymentCode,
                                        decoration: InputDecoration(
                                          labelText:
                                              'Select Payment Mode for Remaining Balance *',
                        ...
                        child: Row(
                          children: [
                            ...
                            IconButton(
                              ...
                              onPressed: () => setState(() => _paymentError = null),
                            ),
                          ],
                        ),
                      ),
                              ...
                              onPressed: () async {
                                if (selectedPrimaryPaymentCode != null) {
                                  ...
                                  if (rem > 0.01 &&
                                    ...
                                    setState(() {
                                      _paymentError =
                                          'Please select a payment mode for the remaining balance of Rs. ${rem.toStringAsFixed(2)}';
                                    });
                                    return;
                                  }

                                  final primaryMode = paymentModes.firstWhere(
                                    (pm) =>
                                        pm['PaymentCode'] ==
                                        selectedPrimaryPaymentCode,
                                    orElse: () => {'PaymentMode': 'Payment'},
                                  )['PaymentMode']?.toString() ?? 'Payment';

                                  selectedPaymentMethodsForPDF = [
                                    {
                                      'mode': primaryMode,
                                      'amount': enteredAmt,
                                    },
                                  ...
                                  if (rem > 0.01 &&
                                      selectedRemainingPaymentCode != null) {
                                    final remainingMode = paymentModes
                                        .firstWhere(
                                          (pm) =>
                                              pm['PaymentCode'] ==
                                              selectedRemainingPaymentCode,
                                    ...
                                    selectedPaymentMethodsForPDF!.add({
                                      'mode': remainingMode,
                                      'amount': rem,
                                    });
                                  }
                                ...
                                await _postSalesOrder(
                                  this.context,
                                  null,
                                  null,
                                  paymentModes: selectedPaymentMethodsForPDF,
                                );
                              },
        ...
        return AlertDialog(
          ...
          content: Column(
            ...
            children: [
              ...
              const SizedBox(height: 4),
              Text('Remarks: $remarks', style: theme.textTheme.bodyMedium),
  ...
  /// Remarks shown in the header field and post-dialog General Remarks.
  /// Both inputs share [_remarksController]; this is what is posted to the DB.
  String _remarksForDatabase() {
    final text = _remarksController.text.trim();
    // Stored procedure p_Remarks is VarChar(100)
  ...
  String _readQuotationDocumentNo(Map<String, dynamic> quotation) {
    for (final key in ['DocumentNo', 'documentNo', 'id']) {
      final value = quotation[key]?.toString().trim();
  ...
  Map<String, dynamic> _mapQuotationDetailToRow(
  ...
  List<Map<String, dynamic>> _quotationDetailsToRows(
    List<Map<String, dynamic>> details,
    String locaCode,
  ) {
    return details
        .map((detail) => _mapQuotationDetailToRow(detail, locaCode))
        .where((row) => row['code'].toString().isNotEmpty)
        .toList();
  }

  Future<Map<String, String>?> _showQuotationRecallDialog({
    required String customerName,
    required List<Map<String, String>> quotations,
  }) async {
    if (quotations.isEmpty) {
      if (!mounted) return null;
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text('No posted quotations found for $customerName'),
          backgroundColor: _accentColor,
        ),
      );
      return null;
  ...
  Future<void> _recallQuotationForSalesOrder() async {
    if (_selectedCustomer == null) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
    ...
    try {
      ...
      final options = quotations
          .map((quotation) {
            final documentNo = _readQuotationDocumentNo(quotation);
            final amount = (quotation['NetAmount'] as num?)?.toDouble() ?? 0.0;
            return {
              'id': documentNo,
              'date': quotation['DocumentDate']?.toString() ?? '',
              'amount': 'Rs. ${amount.toStringAsFixed(2)}',
              'netAmount': amount.toStringAsFixed(2),
              'locaCode': quotation['LocaCode']?.toString() ?? '01',
              'costCenter': quotation['CostCenter']?.toString() ?? '000001',
            };
          })
          .where((quotation) => quotation['id']!.isNotEmpty)
          .map((quotation) => quotation.map((k, v) => MapEntry(k, v.toString())))
          .toList();

      final selected = await _showQuotationRecallDialog(
        customerName: customerName,
        quotations: options,
      );
      ...
      final quotationData = await ApiService.getQuotationForRecall(
        documentNo: quotationNo,
        customerCode: customerCode,
      );
      final header = Map<String, dynamic>.from(
        quotationData['header'] as Map? ?? {},
      );
      final details = (quotationData['details'] as List? ?? [])
          .map((item) => Map<String, dynamic>.from(item as Map))
          .toList();
      final quotationLocaCode = _resolveLocaCode(
        header['LocaCode']?.toString() ?? selected['locaCode'],
      );
      final rows = _quotationDetailsToRows(details, quotationLocaCode);
      ...
      if (rows.isEmpty) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(
              quotationNo.isNotEmpty
                  ? 'Quotation $quotationNo has no line items.'
                  : 'Selected quotation has no line items.',
            ),
            backgroundColor: _accentColor,
          ),
        );
      ...
      setState(() {
        ...
        _recalledQuotationNo = quotationNo.isNotEmpty ? quotationNo : null;
        _rows
          ..clear()
          ..addAll(rows);
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(
            quotationNo.isNotEmpty
                ? 'Quotation $quotationNo loaded with ${rows.length} item(s).'
                : 'Quotation loaded with ${rows.length} item(s).',
          ),
          backgroundColor: _accentColor,
        ),
      );
                          ...
                          return Theme(
                            data: Theme.of(context).copyWith(
                              colorScheme: ColorScheme.light(
                                primary: const Color(0xFF2362EC),
              ...
              children: [
                Expanded(
                  child: ElevatedButton.icon(
                    onPressed: _selectedCustomer == null || _isRecallingQuotation
                        ? null
                        : _recallQuotationForSalesOrder,
                        ...
                        child: DataTable(
                          ...
                          rows: () {
                            ...
                            return _rows.asMap().entries.map((entry) {
                              ...
                              // Calculate item-wise discount amount
                              final discountStr =
  ...
  List<Map<String, dynamic>> _buildSalesOrderTempTransactions({
    ...
    return _rows.asMap().entries.map((entry) {
      ...
      final itemStr = row['item'] as String? ?? '';
  ...
  Future<void> _postSalesOrder(
    ...
    try {
      ...
      final postData = {
        ...
        // Header Remarks + General Remarks share _remarksController — one value to DB.
        'remarks': (() {
          String text = _remarksController.text.trim();
          if (paymentModes != null && paymentModes.isNotEmpty) {
            final paymentText = paymentModes.map((m) {
              final mode = m['mode'] ?? '';
              final amt = m['amount'];
              final amtStr = amt is num ? amt.toStringAsFixed(2) : amt.toString();
              return '$mode: Rs. $amtStr';
            }).join(', ');
            if (text.isEmpty) {
              text = paymentText;
            } else {
              text = '$text | $paymentText';
            }
          }
          if (text.length <= 100) return text;
          return text.substring(0, 100);
      ...
      if (response['success'] == true) {
        ...
        final savedPaymentModes = paymentModes;
                        ...
                        onPressed: () async {
                          ...
                          try {
                            await _generateSalesOrderPDF(
                              ...
                              paymentModes: savedPaymentModes,
                            );
                          } catch (e) {
                            if (mounted) {
