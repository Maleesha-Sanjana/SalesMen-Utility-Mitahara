...
class CrnPage extends StatefulWidget {
  const CrnPage({super.key, this.appBarLeading});

  final Widget? appBarLeading;

  @override
  State<CrnPage> createState() => _CrnPageState();
}
...
class _CrnPageState extends State<CrnPage> with WidgetsBindingObserver {
  static const String _activeDraftKey = 'crn_active_draft';

  String _salesType = 'Retail';
  final _manualController = TextEditingController();
  final _remarksController = TextEditingController();
  final List<Map<String, dynamic>> _rows = [];
  Map<String, String>? _selectedInvoice;
  String? _recalledInvoiceNo;
  double _selectedInvoiceAmount = 0.0;
  Map<String, String>? _selectedCustomer;

  double _crnDiscount = 0.0;
  ...
  bool _isSyncingOfflineCRNs = false;
  bool _isRecallingInvoice = false;
  int _pendingCRNCount = 0;
  String? _activeTempDocNo;
  Timer? _draftPersistTimer;
  bool _suppressDraftPersist = false;
  Color get _accentColor => const Color(0xFFEF4444);
  ...
  Future<void> _loadPendingCRNCount() async {
    final count = await OfflineSyncService.getPendingCRNCount();
    if (!mounted) return;
    setState(() => _pendingCRNCount = count);
  }

  Future<void> _syncPendingCRNs() async {
    setState(() => _isSyncingOfflineCRNs = true);

    try {
      final result = await OfflineSyncService.syncPendingCRNs();
      await _loadPendingCRNCount();
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(
            result.message ??
                'CRN sync: ${result.synced} synced, ${result.failed} failed, ${result.remaining} pending.',
          ),
        ),
      );
    } finally {
      if (mounted) setState(() => _isSyncingOfflineCRNs = false);
    }
  }
  ...
  Map<String, dynamic>? _buildActiveDraftData() {
    ...
    return {
      ...
      'recalledInvoiceNo': _recalledInvoiceNo,
      'selectedInvoiceAmount': _selectedInvoiceAmount,
      'invoiceReturnItems': _invoiceReturnItems.map(_deepCopyMap).toList(),
      'activeTempDocNo': _activeTempDocNo,
    };
  }

  bool get _shouldKeepActiveDraft {
    if (_rows.isNotEmpty) return true;
    return _selectedCustomer != null && _invoiceReturnItems.isNotEmpty;
  }
  ...
  Future<void> _persistActiveDraft({bool immediate = false}) async {
    ...
    Future<void> writeDraft() async {
      try {
        ...
        if (draft == null || !_shouldKeepActiveDraft) {
  ...
  Future<void> _restoreActiveDraft() async {
    try {
      ...
      final savedReturnItems = data['invoiceReturnItems'];
      final rowsList = savedRows is List ? savedRows : const [];
      final returnList = savedReturnItems is List ? savedReturnItems : const [];
      if (rowsList.isEmpty && returnList.isEmpty) return;
      ...
      setState(() {
        ...
        _rows
          ..clear()
          ..addAll(
            rowsList.map((e) => Map<String, dynamic>.from(e as Map)),
          );
        _invoiceReturnItems
          ..clear()
          ..addAll(
            returnList.map((e) => Map<String, dynamic>.from(e as Map)),
          );
        _crnDiscount = (data['discount'] as num?)?.toDouble() ?? 0.0;
        ...
        _recalledInvoiceNo = data['recalledInvoiceNo'] as String?;
        _selectedInvoiceAmount =
            (data['selectedInvoiceAmount'] as num?)?.toDouble() ?? 0.0;
        _activeTempDocNo = data['activeTempDocNo'] as String?;
        if (data['selectedInvoice'] is Map) {
          _selectedInvoice = (data['selectedInvoice'] as Map).map(
            (key, value) => MapEntry(key.toString(), value?.toString() ?? ''),
          );
        } else {
  ...
  Future<void> _saveTemporaryData() async {
    ...
    try {
      ...
      final key = 'crn_temp_$customerId';
  ...
  // Invoice line items available for return (populated after invoice recall)
  List<Map<String, dynamic>> _invoiceReturnItems = [];
  ...
  Map<String, dynamic> _mapInvoiceDetailToReturnItem(
    ...
    return {
      ...
      'price': _salesType == 'Retail' ? unitPrice : wholeSalePrice,
  ...
  List<Map<String, dynamic>> _invoiceDetailsToReturnItems(
    List<Map<String, dynamic>> details,
    String locaCode,
  ) {
    return details
        .map((detail) => _mapInvoiceDetailToReturnItem(detail, locaCode))
        .where((item) => item['code'].toString().isNotEmpty)
        .toList();
  }

  double _usedReturnQty(
    String productCode, {
    List<Map<String, dynamic>>? extraPending,
  }) {
    var used = _rows
        .where((row) => row['code']?.toString() == productCode)
        .fold<double>(
          0,
          (sum, row) => sum + ((row['qty'] as num?)?.toDouble() ?? 0),
        );
    if (extraPending != null) {
      used += extraPending
          .where((entry) => entry['code']?.toString() == productCode)
          .fold<double>(
            0,
            (sum, entry) => sum + ((entry['qty'] as num?)?.toDouble() ?? 0),
          );
    }
    return used;
  }

  double _remainingReturnQty(
    Map<String, dynamic> item, {
    List<Map<String, dynamic>>? extraPending,
  }) {
    final productCode = item['code']?.toString() ?? '';
    final invoiceQty = (item['invoiceQty'] as num?)?.toDouble() ?? 0.0;
    return invoiceQty - _usedReturnQty(productCode, extraPending: extraPending);
  }

  List<Map<String, dynamic>> _availableReturnItems({
    List<Map<String, dynamic>>? extraPending,
  }) {
    return _invoiceReturnItems
        .where((item) => _remainingReturnQty(item, extraPending: extraPending) > 0)
        .toList();
  }
  ...
  void dispose() {
    ...
    final draft = _buildActiveDraftData();
    final encoded = (draft != null && _shouldKeepActiveDraft)
        ? jsonEncode(draft)
        : null;
  ...
  _DiscountValidationResult _validateDiscount({
    ...
    List<Map<String, dynamic>>? extraPendingEntries,
  }) {
    ...
    final invoiceQty = (selectedItem['invoiceQty'] as num?)?.toDouble();
    if (invoiceQty != null && invoiceQty > 0) {
      final remaining = _remainingReturnQty(
        selectedItem,
        extraPending: extraPendingEntries,
      );
      if (quantity > remaining) {
        return _DiscountValidationResult(
          errorMessage: remaining <= 0
              ? 'All invoice quantity for this item has already been added.'
              : 'Return qty cannot exceed remaining invoice qty (${remaining.toStringAsFixed(0)}).',
        );
      }
    }
                        ...
                        FilledButton(
                          onPressed: () {
                            ...
                            if (amount == null || amount <= 0) {
                              setDialogState(() { paymentError = 'Please enter a valid amount'; });
                              return;
                            }
                            if (amount > totalAmount) {
                              setDialogState(() { paymentError = 'Amount cannot exceed total: Rs. ${totalAmount.toStringAsFixed(2)}'; });
                              return;
                            }
                            if (hasRemaining &&
                                selectedRemainingPaymentMethod == null) {
                              setDialogState(() { paymentError = 'Please select a payment method for the remaining amount'; });
                              return;
                            }
                        ...
                        FilledButton(
                          onPressed: () {
                            if (cardNumberController.text.trim().isEmpty) {
                              setDialogState(() { paymentError = 'Please enter $cardName number'; });
                              return;
                            }
                            final amount = double.tryParse(
                              amountController.text,
                        ...
                        FilledButton(
                          onPressed: () {
                            if (chequeNumberController.text.trim().length !=
                                6) {
                              setDialogState(() { paymentError = 'Cheque number must be 6 digits'; });
                              return;
                            }
                            if (bankController.text.trim().isEmpty) {
                              setDialogState(() { paymentError = 'Please enter bank name'; });
                              return;
                            }
                            if (branchController.text.trim().isEmpty) {
                              setDialogState(() { paymentError = 'Please enter branch name'; });
                              return;
                            }
                            if (chequeDate == null) {
                              setDialogState(() { paymentError = 'Please select cheque date'; });
                              return;
                            }
                            final amount = double.tryParse(
                              amountController.text,
                        ...
                        FilledButton(
                          onPressed: () {
                            if (codNumberController.text.trim().isEmpty) {
                              setDialogState(() { paymentError = 'Please enter COD number'; });
                              return;
                            }
                            final amount = double.tryParse(
                              amountController.text,
                        ...
                        FilledButton(
                          onPressed: () {
                            if (depositNumberController.text.trim().isEmpty) {
                              setDialogState(() { paymentError = 'Please enter deposit number'; });
                              return;
                            }
                            if (bankController.text.trim().isEmpty) {
                              setDialogState(() { paymentError = 'Please enter bank name'; });
  ...
  Future<void> _openPostCRNDialog() async {
    ...
    if (_rows.isEmpty) {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: const Text('Please add items to the crn before posting'),
            backgroundColor: _accentColor,
            duration: const Duration(seconds: 3),
          ),
        );
                          ...
                          Expanded(
                            ...
                            child: FilledButton.icon(
                              onPressed: () async {
                                ...
                                // Post crn using stored procedure
                                Navigator.of(
                                  context,
  ...
  Future<void> _openAddItemDialog() async {
    ...
    if (_recalledInvoiceNo == null || _recalledInvoiceNo!.isEmpty) {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: const Text('Please recall an invoice first'),
            backgroundColor: _accentColor,
            duration: const Duration(seconds: 3),
          ),
        );
    ...
    if (_invoiceReturnItems.isEmpty) {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: const Text('Recalled invoice has no items to return'),
            backgroundColor: _accentColor,
            duration: const Duration(seconds: 3),
          ),
        );
    ...
    final result = await showDialog<Map<String, dynamic>?>(
      ...
      builder: (dialogContext) {
        ...
        return StatefulBuilder(
          builder: (dialogContext, setDialogState) {
            final returnItems = _availableReturnItems(
            ...
            void refreshSummary() {
              ...
              final validation = _validateDiscount(
                ...
                extraPendingEntries: pendingEntries,
              );
            ...
            return AlertDialog(
              title: Text('Add Return Item (Invoice $_recalledInvoiceNo)'),
                          ...
                          ? Center(
                              child: Column(
                                ...
                                children: [
                                  ...
                                  const SizedBox(height: 8),
                                  Text(
                                    'All invoice items have been added',
                                    style: theme.textTheme.bodyLarge,
                                  ),
                                ],
                              ),
                                          ...
                                          children: [
                                            ...
                                            Text(
                                              hasQuery
                                                  ...
                                                  : 'All items from this invoice may already be added',
                                              style: theme.textTheme.bodySmall
                                                  ?.copyWith(
                                                    color: theme
                                                        .colorScheme
                                          ...
                                          child: Container(
                                            ...
                                            child: Column(
                                              ...
                                              children: [
                                                ...
                                                Text(
                                                  'Inv Qty: ${(item['invoiceQty'] as num?)?.toStringAsFixed(0) ?? '0'} • Rs ${_getPriceForItem(item).toStringAsFixed(2)}',
                                                  style: theme
                                                      .textTheme.labelSmall
                                                      ?.copyWith(
                                                    color: _accentColor,
  ...
  void _mergeCRNRow(Map<String, dynamic> row) {
  ...
  Future<void> _showCRNSummary() async {
    if (_rows.isEmpty) {
      return;
    }
    ...
    final invoice = _recalledInvoiceNo ?? 'None';
        ...
        return AlertDialog(
          ...
          content: Column(
            ...
            children: [
              ...
              const SizedBox(height: 4),
              Text('Invoice: $invoice', style: theme.textTheme.bodyMedium),
              const Divider(height: 24),
              ...
              Text(
                'Full Bill Discount: ${_isPercentageDiscount ? '%' : 'Rs'} ${_crnDiscount.toStringAsFixed(2)}',
                style: theme.textTheme.bodyMedium,
              ),
              const SizedBox(height: 4),
              Text(
  ...
  String _readInvoiceDocumentNo(Map<String, dynamic> invoice) {
  ...
  void _clearRecalledInvoice() {
    _selectedInvoice = null;
    _recalledInvoiceNo = null;
    _selectedInvoiceAmount = 0.0;
    _invoiceReturnItems = [];
  }

  Future<Map<String, String>?> _showInvoiceRecallDialog({
    ...
    if (invoices.isEmpty) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text('No posted invoices found for $customerName'),
          backgroundColor: _accentColor,
        ),
      );
      return null;
  ...
  Future<void> _recallInvoiceForCrn() async {
    if (_selectedCustomer == null) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: const Text('Please select a customer first'),
    ...
    setState(() => _isRecallingInvoice = true);

    try {
      final invoices = await ApiService.getInvoicesForRecall(
        customerCode: customerCode,
      );

      final options = invoices
          .map((invoice) {
            final documentNo = _readInvoiceDocumentNo(invoice);
            final amount = (invoice['NetAmount'] as num?)?.toDouble() ?? 0.0;
            return {
              'id': documentNo,
              'date': invoice['DocumentDate']?.toString() ?? '',
              'amount': 'Rs. ${amount.toStringAsFixed(2)}',
              'netAmount': amount.toStringAsFixed(2),
              'locaCode': invoice['LocaCode']?.toString() ?? '01',
              'costCenter': invoice['CostCenter']?.toString() ?? '000001',
            };
          })
          .where((invoice) => invoice['id']!.isNotEmpty)
          .map((invoice) => invoice.map((k, v) => MapEntry(k, v.toString())))
          .toList();

      final selected = await _showInvoiceRecallDialog(
        customerName: customerName,
        invoices: options,
      );
      ...
      if (selected.isEmpty) {
        setState(_clearRecalledInvoice);
        return;
      }

      final invoiceNo = selected['id']?.trim() ?? '';
      if (invoiceNo.isEmpty) {
        setState(_clearRecalledInvoice);
        return;
      }

      final invoiceData = await ApiService.getInvoiceForRecall(
        documentNo: invoiceNo,
        customerCode: customerCode,
      );
      final header = Map<String, dynamic>.from(
        invoiceData['header'] as Map? ?? {},
      );
      final details = (invoiceData['details'] as List? ?? [])
          .map((item) => Map<String, dynamic>.from(item as Map))
          .toList();
      final locaCode =
          header['LocaCode']?.toString() ??
          selected['locaCode']?.trim() ??
          '01';
      final returnItems = _invoiceDetailsToReturnItems(details, locaCode);

      if (returnItems.isEmpty) {
        if (!mounted) return;
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('Invoice $invoiceNo has no line items to return.'),
            backgroundColor: _accentColor,
          ),
        );
        return;
      ...
      setState(() {
        ...
        _recalledInvoiceNo = invoiceNo;
        _selectedInvoiceAmount =
            double.tryParse(selected['netAmount'] ?? '') ?? 0.0;
        _invoiceReturnItems = returnItems;
        _rows.clear();
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(
            'Invoice $invoiceNo loaded with ${returnItems.length} item(s) for return.',
          ),
          backgroundColor: _accentColor,
        ),
      );
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text('Failed to recall invoice: $e'),
          backgroundColor: Colors.red,
        ),
      );
    } finally {
      if (mounted) setState(() => _isRecallingInvoice = false);
    }
  }
  ...
  Future<void> _openDiscountTaxesDialog() async {
    ...
    final discountCtrl = TextEditingController(text: _crnDiscount.toString());
              ...
              children: [
                Expanded(
                  child: ElevatedButton.icon(
                    onPressed: _selectedCustomer == null || _isRecallingInvoice
                        ? null
                        : _recallInvoiceForCrn,
                    icon: _isRecallingInvoice
                        ? SizedBox(
                            width: 18,
                            height: 18,
                    ...
                    label: Text(
                      (_recalledInvoiceNo?.isNotEmpty ?? false)
                          ? _recalledInvoiceNo!
                          : 'Recall Invoice',
                      overflow: TextOverflow.ellipsis,
                    ),
              ...
              children: [
                Expanded(
                  child: ElevatedButton.icon(
                    onPressed:
                        (_recalledInvoiceNo?.isNotEmpty ?? false) &&
                            _invoiceReturnItems.isNotEmpty
                        ? _openAddItemDialog
                        : null,
  ...
  // Generate PDF for crn (delegates to CRNPdfGenerator)
  Future<void> _generateCRNPDF({
  ...
  Future<void> _postCRN(
    ...
    if (_rows.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('Please add items to the crn'),
          backgroundColor: Colors.red,
        ),
      );
      return;
    ...
    try {
      ...
      offlinePostData = {
        ...
        'discPer': _isPercentageDiscount ? _crnDiscount : 0.0,
        ...
        'invoice': _recalledInvoiceNo ?? '',
        'invRecall': _recalledInvoiceNo != null && _recalledInvoiceNo!.isNotEmpty,
        'invAmount': _selectedInvoiceAmount,
        'returnType': 'Customer Return',
        'tempCreditAmt': netAmount,
      };
      ...
      final postResult = await ApiService.postCRNPayload(offlinePostData);

      final crnNo = postResult['documentNo']?.toString() ?? tempDocNo;
      ...
      if (offlineTempTransactions == null ||
        ...
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: Text('Failed to prepare offline crn: $e'),
              backgroundColor: Colors.red,
            ),
          );
        }
      ...
      final localDocNo = await OfflineSyncService.queueCRNPost(
      ...
      if (mounted) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('Offline area. CRN $localDocNo saved locally.'),
            action: SnackBarAction(
              label: 'Sync Now',
              onPressed: _syncPendingCRNs,
            ),
          ),
        );
      }
